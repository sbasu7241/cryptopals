...
class HarderECBOracle(ECBOracle):
    ...
    def __init__(self, secret_padding):
        ...
        self._random_prefix = Random.new().read(randint(0, 255)) # random_prefix of variable length
...
def get_next_byte(prefix_length, block_length, curr_decrypted_message, encryption_oracle):
    ...
    """
    # Initially current decrypted message is ''.  Say [aaaaaaaaaaaaaaaabcdef](21=16+5) + [input]() + [secretstringtobediscovered](24=16+8) + padding[]
...
def find_prefix_length(encryption_oracle, block_length):
    ...
    ciphertext1 = encryption_oracle.encrypt(b'')	#Cipher1 is ECB_encrypt(random_prefix+target_msg)
    ciphertext2 = encryption_oracle.encrypt(b'a')	#Cipher2 is ECB_encrypt(random_prefix+'a'+target_msg)
...
def byte_at_a_time_ecb_decryption_harder(encryption_oracle):
    ...
    prefix_length = find_prefix_length(encryption_oracle, block_length)		#prefix_length is obtained
    mysterious_text_length = len(encryption_oracle.encrypt(b'')) - prefix_length	#mysterious text length obtained
